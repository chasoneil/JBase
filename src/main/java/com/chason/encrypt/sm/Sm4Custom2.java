package com.chason.encrypt.sm;

import java.nio.charset.StandardCharsets;
import java.util.Arrays;

public class Sm4Custom2 {

    private static final int BLOCK_SIZE = 16;
    private static final int ROUND_COUNT = 32;

    private static final int[] SBOX = {
            
    };

    private static final int[] CK = {
            0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
            0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
            0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
            0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
            0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
            0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
            0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
            0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
    };

    private static final int[] FK = {
            0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC
    };

    private int[] rk = new int[ROUND_COUNT];

    public void setKey(byte[] key) {
        if (key.length != 16) {
            throw new IllegalArgumentException("Key length must be 16 bytes.");
        }
        int[] MK = toIntArray(key);
        int[] K = new int[4];
        for (int i = 0; i < 4; i++) {
            K[i] = MK[i] ^ FK[i];
        }
        for (int i = 0; i < ROUND_COUNT; i++) {
            K[i % 4] ^= T(K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i]);
            rk[i] = K[i % 4];
        }
    }

    public byte[] encrypt(byte[] input) {
        byte[] padded = pad(input);
        int[] X = toIntArray(padded);
        X = crypt(X, true);
        return toByteArray(X);
    }

    public byte[] decrypt(byte[] input) {
        int[] X = toIntArray(input);
        X = crypt(X, false);
        byte[] decrypted = toByteArray(X);
        return unpad(decrypted);
    }

    private int[] crypt(int[] X, boolean encrypt) {
        for (int i = 0; i < ROUND_COUNT; i++) {
            int r = encrypt ? i : (ROUND_COUNT - 1 - i);
            X[i % 4] ^= T(X[(i + 1) % 4] ^ X[(i + 2) % 4] ^ X[(i + 3) % 4] ^ rk[r]);
        }
        return X;
    }

    private int T(int B) {
        return L(tau(B));
    }

    private int tau(int B) {
        return (SBOX[(B >>> 24) & 0xFF] << 24)
                | (SBOX[(B >>> 16) & 0xFF] << 16)
                | (SBOX[(B >>> 8) & 0xFF] << 8)
                | (SBOX[B & 0xFF]);
    }

    private int L(int B) {
        return B ^ (B << 2 | B >>> 30) ^ (B << 10 | B >>> 22) ^ (B << 18 | B >>> 14) ^ (B << 24 | B >>> 8);
    }

    private int[] toIntArray(byte[] b) {
        int[] result = new int[b.length / 4];
        for (int i = 0; i < result.length; i++) {
            result[i] = ((b[4 * i] & 0xFF) << 24) | ((b[4 * i + 1] & 0xFF) << 16)
                    | ((b[4 * i + 2] & 0xFF) << 8) | (b[4 * i + 3] & 0xFF);
        }
        return result;
    }

    private byte[] toByteArray(int[] x) {
        byte[] result = new byte[x.length * 4];
        for (int i = 0; i < x.length; i++) {
            result[4 * i] = (byte) (x[i] >>> 24);
            result[4 * i + 1] = (byte) (x[i] >>> 16);
            result[4 * i + 2] = (byte) (x[i] >>> 8);
            result[4 * i + 3] = (byte) x[i];
        }
        return result;
    }

    private byte[] pad(byte[] input) {
        int paddingLength = BLOCK_SIZE - (input.length % BLOCK_SIZE);
        byte[] padded = new byte[input.length + paddingLength];
        System.arraycopy(input, 0, padded, 0, input.length);
        Arrays.fill(padded, input.length, padded.length, (byte) paddingLength);
        return padded;
    }

    private byte[] unpad(byte[] input) {
        int paddingLength = input[input.length - 1] & 0xFF;
        byte[] unpadded = new byte[input.length - paddingLength];
        System.arraycopy(input, 0, unpadded, 0, unpadded.length);
        return unpadded;
    }

    public static void main(String[] args) {
        Sm4Custom2 sm4 = new Sm4Custom2();
        byte[] key = "0123456789ABCDEF".getBytes(StandardCharsets.UTF_8);
        byte[] plaintext = "Hello".getBytes(StandardCharsets.UTF_8);

        sm4.setKey(key);

        byte[] ciphertext = sm4.encrypt(plaintext);
        System.out.println("Ciphertext: " + Arrays.toString(ciphertext));

        byte[] decrypted = sm4.decrypt(ciphertext);
        System.out.println("Decrypted: " + new String(decrypted, StandardCharsets.UTF_8));
    }

}
