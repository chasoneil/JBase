...
&emsp;&emsp;在第一种实现方式中，我们是将线程的创建和线程执行的业务都封装在了Thread对象中，我们可以通过Runnable接口来实现线程程序代码和数据有效的分离。
...
1. 创建Runnable的实现类
2. 重写run方法
3. 创建Runnable实例对象(通过实现类来实现)
4. 创建Thread对象，并把第三部的Runnable实现作为Thread构造方法的参数
5. 启动线程
...
使用多线程实现一个抢票程序，要求：票的数量有限，票为0的时候就不能抢了，提示无票；输出每个线程抢票的明细。
...
其实Callable接口底层的实现就是对Runnable接口实现的封装，线程启动执行的也是Runnable接口实现中的run方法，只是在run方法中有调用call方法罢了
