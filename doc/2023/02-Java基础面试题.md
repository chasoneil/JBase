...
一共有六种 https://www.modb.pro/db/136334

**SingleThreadScheduledExecutor** : 他是定时任务线程池和单线程线程池的合并

**ForkJoinPool** : 这是一个非常适合将任务拆分的线程池，如果你的任务非常繁重，但是你可以将任务拆分成很多个互不影响的子任务，最终再做聚合，那么这个线程池就会非常适合你
...
> 线程池的 execute() 和 submit() 的区别

execute 只能用于提交实现了Runnable的线程任务，这意味着这个方法是没有返回值的。

submit 可以用于提交实现了Callable的任务，这意味着 submit 可以执行有返回值的任务，

当线程内部执行发生异常 submit 可以通过 FutureTask.get() 获取异常并处理， execute 则是线程内部直接吞掉异常
...
线程池的执行流程就是 execute() 的执行流程

![image-20230615162416177](C:\Users\86183\AppData\Roaming\Typora\typora-user-images\image-20230615162416177.png)
