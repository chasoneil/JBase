...
new CountDownLatch时，直接指定好具体的数值。这个数值会赋值给state属性。
...
比如要求当前服务最多10个线程同时干活，将信号量设置为10。每一个任务提交都需要获取一个信号量，就去干活，干完了，归还信号量。
...
它相当于一个线程安全的ArrayList , 不同于Vector， 他是通过 ReentrantLock保证线程安全的
...
回答3：这里ConcurrentHashMap是采用LongAdder实现的计数，底层还是CAS。（AtomicLong）
